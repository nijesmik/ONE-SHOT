...
CREATE TABLE IF NOT EXISTS `user` ( -- 유저 정보를 저장하는 테이블
  ...
  `user_name` VARCHAR(256) COMMENT '실제 유저 이름',
  `user_phonenumber` VARCHAR(256) COMMENT '유저 연락처',
  ...
  `info` TEXT COMMENT '유저 특이 사항 기입 용 주석 역할 column',
...
INSERT INTO user
(user_code, user_email, password, nick_name, user_name, user_phonenumber, admin, service)
VALUES
("000000", "ssafy@ssafy.com", "1234", "비회원", "비회원", "010-0000-0000", 0, 1),
("000001", "junghoon1039@gmail.com", "1234", "관리자-박정훈", "박정훈", "010-9497-8181", 1, 1),
("000002", "sejin@gmail.com", "1234", "관리자-김세진", "김세진", "010-0000-0000", 1, 1)
;
...
CREATE TABLE IF NOT EXISTS `brand` ( -- 브랜드 정보를 저장하는 테이블
  ...
  `brand_name_ko` VARCHAR(256) NOT NULL COMMENT '브랜드 한글 이름',
  `brand_name_en` VARCHAR(256) COMMENT '브랜드 영어 이름',
  ...
  `info` TEXT COMMENT '브랜드 특이 사항 기입 용 주석 역할 column',
...
CREATE TABLE IF NOT EXISTS `menu` ( -- 메뉴 정보를 저장하는 테이블 | 브랜드에 귀속
  ...
  `menu_name_ko` VARCHAR(256) NOT NULL COMMENT '메뉴 한글 이름',
  `menu_name_en` VARCHAR(256) COMMENT '메뉴 영어 이름',
  ...
  `info` TEXT COMMENT '메뉴 특이 사항 기입 용 주석 역할 column',
...
CREATE TABLE IF NOT EXISTS `result` ( -- 개별 주문의 종합 결과서 정보 테이블 | 유저에게 귀속
  `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '결과서 기본 PK',
  `result_code` VARCHAR(256) NOT NULL UNIQUE KEY COMMENT '결과서 고유 번호',
  `user_code` VARCHAR(256) NOT NULL COMMENT '결과서 생성 유저 | 비회원일 경우 고정값을 가짐',
  `result_url` VARCHAR(256) NOT NULL COMMENT '결과서 공유 url',
  `order_list` TEXT COMMENT '결과서에 속한 주문들의 정보 | JSON 배열 형태',
  `total_price` INT COMMENT '결과서 주문들의 총 금액 합',
  `service` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '결과서 서비스 노출 여부 : 0 노출 안함 | 1 노출',
  `info` TEXT COMMENT '결과서 특이 사항 기입 용 주석 역할 column',
  `created_time`  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '결과서 row 생성 시각',
  `updated_time`  TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '마지막 결과서 row 수정 시각',
  CONSTRAINT `fk_result_user_code` FOREIGN KEY (`user_code`) REFERENCES `user` (`user_code`) -- 유저코드 외래키
) ENGINE=InnoDB DEFAULT CHARACTER SET = utf8mb4;

CREATE TABLE IF NOT EXISTS `order` ( -- 개별 주문 정보 테이블 | 결과서에 귀속
  `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '주문 기본 PK',
  `order_code` VARCHAR(256) NOT NULL UNIQUE KEY COMMENT '주문 고유 번호',
  `result_code` VARCHAR(256) NOT NULL COMMENT '주문이 속한 결과서의 고유 번호',
  ...
  `info` TEXT COMMENT '주문 특이 사항 기입 용 주석 역할 column',
  ...
  CONSTRAINT `fk_order_result_code` FOREIGN KEY (`result_code`) REFERENCES `result` (`result_code`), -- 결과서코드 외래키
  CONSTRAINT `fk_order_menu_code` FOREIGN KEY (`menu_code`) REFERENCES `menu` (`menu_code`) -- 메뉴코드 외래키
) ENGINE=InnoDB DEFAULT CHARACTER SET = utf8mb4;
